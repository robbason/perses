...
import openforcefield.topology as openff_topology
...
def _add_method(object, function):
        """
...
def _set_residue_mol_and_openmm_topology_attributes(topology, residue_mol, residue_topology, residue_to_mol_map):
    """
    Add the following attributes to an openmm.Topology:
        current openmm topology with a residue mol,
    ...
    residue_mol : Molecule
        mol of the residue of interest
    residue_topology : simtk.openmm.Topology.residue
        the residue of interest
    residue_to_mol_map : dict
        dictionary of the residue_topology indices to the residue_mol indices

    NOTE: the atoms comprising the residue_topology must be a subset of the residue_mol atoms
    """
    assert {atom.name for atom in residue_topology.atoms()}.issubset({atom.name for atom in residue_mol.atoms}), f"the self.residue_topology is not a subset of the self.residue_mol"
    for attribute, name in zip([residue_mol, residue_topology, residue_to_mol_map], ['residue_mol', 'residue_topology', 'residue_to_mol_map']):
        setattr(topology, name, attribute)

    reverse_residue_to_oemol_map = {val : key for key, val in residue_to_mol_map.items()}
...
def augment_openmm_topology(topology, residue_mol, residue_topology, residue_to_mol_map):
    """
    ...
    residue_to_mol_map : dict
        dictionary of the residue_topology indices to the residue_mol indices
    """
    # TODO: a better way would be to create a top-level container object containing the topology as well as these additional attributes. You _can_ add attributes and methods to the openmm Topology class, but that doesn't mean you should!
    _set_residue_mol_and_openmm_topology_attributes(topology, residue_mol, residue_topology, residue_to_mol_map)
    _add_method(topology, _get_networkx_molecule)
...
class TopologyProposal:
    ...
    def __init__(self,
                 ...
                 metadata=None):
            # TODO: group variables under _new, _old holders.
            #Maybe use namedtuples?
...
class PolymerProposalEngine(ProposalEngine):
    ...
    def _construct_atom_map(self,
        ...
        local_atom_map_nonstereo_sidechain = atom_mapper._get_mol_atom_map(current_oemol, proposed_oemol, map_strength='default', matching_criterion='name', map_strategy='matching_criterion', allow_ring_breaking=break_bool)
        ...
        if local_atom_map_nonstereo_sidechain is not None:
            local_atom_map_stereo_sidechain = atom_mapper.preserve_chirality(current_oemol, proposed_oemol, local_atom_map_nonstereo_sidechain)
        else:
...
class SmallMoleculeSetProposalEngine(ProposalEngine):
    ...
    def _topology_to_smiles(self, topology):
        ...
        # Find residue in topology by searching for residues with name "MOL"
        matching_molecules = [res for res in topology.residues() if res.name[:3] == molecule_name[:3]]
        ...
        # TODO: options to pass into to_smiles?
        return mol_res.to_smiles(), mol_res
    ...
    def _build_new_topology(self, current_receptor_topology, mol_proposed):
        """
        Construct a new topology
        Parameters
        ----------
        mol_proposed : Molecule
            the proposed Molecule object
        ...
        """
        _logger.info(f"\tsetting proposed mol title to {self._residue_name}")
        mol_proposed.name = self._residue_name
        # TODO: Can perses use openforcefield topology exclusively and not openmm topology?
        new_openff_topology = openff_topology.Topology.from_openmm(current_receptor_topology)

        _logger.info(f"\tcreating mol topology from mol...")
        mol_openff_topology = openff_topology.Topology()
        mol_openff_topology.add_molecule(mol_proposed)
        append_topology(new_openff_topology, mol_openff_topology)
        ...
        return new_openff_topology.to_openmm()
